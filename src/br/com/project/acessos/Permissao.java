...
public enum Permissao {
	...
	public static List<Permissao> getListPermissaoComLambda() {
		List<Permissao> permissoesLambda = new ArrayList<>();
		permissoesLambda.forEach(permissao -> {
			permissoesLambda.add(permissao);
		});

		Collections.sort(permissoesLambda, new Comparator<Permissao>() {
			@SuppressWarnings("deprecation")
			@Override
			public int compare(Permissao o1, Permissao o2) {
				return new Integer(o1.ordinal()).compareTo(new Integer(o2.ordinal()));
